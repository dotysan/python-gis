...
if [ ! -d "$DEPOT_TOOLS_DIR" ]; then
  time git clone "$DEPOT_TOOLS_URL" "$DEPOT_TOOLS_DIR" --single-branch
else 
...
time gclient config --unmanaged "$PDFIUM_URL" --custom-var=checkout_configuration=minimal
time gclient sync --revision="$REV" --shallow --no-history
